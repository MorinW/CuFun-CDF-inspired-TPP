...
from CuFun.tpp.nn import BaseModule, LinearTransfer
...
class TransformedDistribution(BaseModule):
    ...
    def log_prob(self, y, h=None, emb=None):
        ...
        log_p += self.base_dist.log_prob(x, h=h, emb=emb)[0]  #############--!!!--##############
        density_integral = self.base_dist.log_prob(x, h=h, emb=emb)[1]
        # log_p += self.base_dist.log_prob(x, h=h, emb=emb)
        return log_p, density_integral
    ...
    def log_fx(self, y, h=None, emb=None):
        log_fx = 0.0
        for transform in reversed(self.transforms):
            x, inv_log_det_jacobian = transform.inverse(y, h=h, emb=emb)
            log_fx += inv_log_det_jacobian
            y = x

        log_fx += self.base_dist.log_prob(x, h=h, emb=emb)
        return log_fx
