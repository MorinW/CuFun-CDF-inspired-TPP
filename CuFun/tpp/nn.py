...
class RNNLayer_ours(BaseModule):
    """RNN for encoding the event history."""
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.history_size
    ...
    def forward(self, input):
        ...
        """
        t_ori = input.in_time
        length_ori = input.length
        length_ori[:] = length_ori[0]
        h_list = list()
        for i in range(t_ori.size(1)):
            t = t_ori[:, i:]
            length = length_ori - i
            if not self.using_history:
                return torch.zeros(t.shape[0], t.shape[1], self.hidden_size)
...
class Hypernet(nn.Module):
    ...
    Args:
        config: Model configuration. See `tpp.model.ModelConfig`.
...
class LinearTransfer(nn.Module):
    ...
    def forward(self, log_fx, item_num):
        linear = nn.Linear(item_num, 1, bias=True)
        intensity = torch.squeeze(linear(torch.exp(log_fx)))
...
class SoftPlus(nn.Module):
    ...
    def forward(self, log_fx):
        fx = torch.exp(log_fx)
        intensity = self.s * torch.log(1+torch.exp(fx/self.s))
        log_intensity = torch.log(intensity)
        return log_intensity
...
class Conv1d(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1d_layers = nn.Sequential( # downsampling factor = 160
            nn.Conv1d(1, 512, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm1d(512),
            nn.ReLU(inplace=True),
            nn.Conv1d(512, 1, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm1d(1),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, log_fx):
        ...
        intensity = self.conv1d_layers(fx)
        intensity = torch.squeeze(intensity)
        log_intensity = torch.log(intensity)
        return log_intensity
